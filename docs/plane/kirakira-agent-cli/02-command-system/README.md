...
The CLI is built on **oclif** with a **pattern-based** command loader:
`packages/cli/package.json` points `"oclif.commands"` at `./dist/commands`, so
each file under `packages/cli/src/commands/` becomes a subcommand tree after
build.

## Fifteen completion topics

Completion generation lists the same canonical set
...
Shared oclif patterns (flags, args) live alongside
**`packages/cli/src/base-command.ts`** when commands need common behavior.
