...
| Runtime profiles | `reference_project/eam-agent/packages/config-resolver`, `agent.toml`, `policy.yaml` | `configs/runtime/profiles.json`, `packages/config-resolver`, `scripts/runtime-profile.mjs`, daemon resolved-config startup, CLI resolver adapter, `scripts/runtime-integration-gate.mjs` | Advancing. Daemon and CLI now consume resolved config, profile projection can emit MCP config plus memory-stack startup fragments without writing local `.mcp.json`, read-only CLI MCP commands now resolve the profile projection before local compatibility overlays, and `integrationGates.upgrade` gives release checks a profile-owned aggregate gate instead of a prose-only checklist. | Move remaining CLI config subcommands and init/doctor surfaces away from local helper duplication, project integration-gate fragments through the resolver, then wire projection fragments into launchers where behavior permits. |
...
- **Runtime ecosystem:** `test/helpers/memory-env.ts` now derives the
  ...
  and workbench child gate evidence into
  `docs/upgrade/gates/runtime-integration-gate.json`. Remaining work is broader
  daemon startup coverage, regular live Electron smoke execution, single-run
  daemon composition smoke coverage, and Compose duplication reduction.
- **Architecture/API:** the largest hardcoding seam is no longer package
  ...
  persistence gate now has isolated live Docker evidence. The aggregate runtime
  integration gate uses adapter injection and profile data instead of a switch
  over child gates. Next work should target topology source-adapter validation,
  telemetry propagation smoke coverage, and per-service readiness.
...
5. **Workbench IA pass:** split `packages/frontend-app/src/workbench.tsx` into
   ...
   through frontend-core. Remaining work is broader screenshot QA and density
   polish.
...
8. **Runtime integration gate:** `pnpm integration:gate` now aggregates
   profile-owned deep-research, memory, and workbench evidence without starting
   live services by default. Remaining work is a stronger single-run
   runtime-daemon composition smoke that proves the same mechanisms compose
   inside one `KernelBridge` execution.
