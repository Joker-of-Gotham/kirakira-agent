...
| Hardcoding, harness, SDK/API | 30% | Runtime env, artifact policy, daemon config, and CLI config loading are being centralized through resolver-backed contracts. | Consolidate SDK result parsing, replace switch-heavy routing where it blocks extensibility, harden test harnesses. |
...
- **Architecture/API:** the largest hardcoding seam is no longer package
  ...
  `@kirakira/config-resolver`; next work should target runtime control result
  parsing and per-service test readiness.
...
2. **Runtime control contract:** export typed ack/result parsers from
   runtime contracts and make daemon client, browser transport, and desktop IPC
