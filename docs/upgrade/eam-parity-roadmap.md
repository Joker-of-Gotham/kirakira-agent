...
- **Runtime ecosystem:** `test/helpers/memory-env.ts` now derives the
  ...
  runtime-daemon composition, workbench smoke, and hydrated visual QA child gate
  evidence into `docs/upgrade/gates/runtime-integration-gate.json`. Remaining
  work is broader daemon lifecycle startup coverage, full Docker-backed live
  web/Electron execution, and Compose duplication reduction.
...
5. **Workbench IA pass:** split `packages/frontend-app/src/workbench.tsx` into
   ...
   for web/desktop and mobile/tablet/desktop. Remaining work is full-stack live
   visual QA and density polish.
...
8. **Runtime integration gate:** `pnpm integration:gate` now aggregates
   ...
   workbench plus hydrated visual QA evidence without starting live services by
   default. The single-run composition smoke now proves the same mechanisms
   compose inside one `KernelBridge` execution, and the presentation gate now
   proves hydrated renderer screenshots. Remaining work is full Docker-backed
   web/Electron smoke execution in a slower environment.
