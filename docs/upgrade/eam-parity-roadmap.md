...
intentional Kirakira extension rows: five covered, three partial, and zero
...
- **Architecture/API:** the largest hardcoding seam is no longer package
  ...
  dependencies now emit typed retain/reflect runtime events. Next work should
  target live memory/topology source-adapter validation, telemetry propagation
  smoke coverage, and per-service readiness.
