...
| `mcp-adapter` | Intentional Kirakira extension | Partial | Gateway context, OTel bridge, and OTel profile tests cover trust/policy/audit metadata, W3C trace metadata, MCP `tools/call` span fields, profile/env-selected recorder plans, SDK/OTLP factory selection, daemon recorder injection, and tool-result errors with `isError`. | A concrete OpenTelemetry SDK/OTLP factory still needs to be injected by daemon/runtime hosts when profiles request SDK-owned export; live propagation smoke coverage remains. |
